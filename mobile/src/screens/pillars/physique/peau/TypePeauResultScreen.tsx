...
export function TypePeauResultScreen() {
  const result = useDiagnosticStore((state) => state.typePeau);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Text style={styles.title}>{microcopy.peau.resultTitle}</Text>
      <Text style={styles.type}>{result.type_peau}</Text>
      <Text style={styles.body}>{result.description}</Text>

      {result.conseils_style.length > 0 ? (
        <View style={styles.conseils}>
          <Text style={styles.conseilsTitle}>{microcopy.peau.conseilsTitle}</Text>
